...
| [ ] | Discover Segments | `segments.md` | `segments.md` | 1. Write the job to be done in the file after knowing the answers above following this structure: "# {job to be done}".<br>2. Run the prompt of `segments.md` to discover segments and write in the new file.<br>3. In the chat list only the titles preceded by sequential numbers, including the first option as "None: Explore more segments", the second one as the market definition(but not mentioning "Market Definition" string), followed by the list of segments .<br>3. Ask the user to choose one to proceed in the chat. 4. If the user selected option 1, "Explore More Segments", run again to find additional segments. |
...
| [ ] | Discover Actors | `actor_roles.md` | `segment_actors.md` | 1. List the roles and actors related to the selected segment, in the result file.<br>2. In the chat list only the titles containing "{role}: {actor}" preceded by sequential number.<br>3. Ask the user to choose one actor to proceed in the chat to market definition. |
| [ ] | Determine Market Definition on focus | none | `segment_job_map.md` | Write the Market Definition on focus in the file after knowing the answers above following this structure: "# Market definition: {actor} trying to to {job to be done}". |
